...
MPESA_INITIATOR_NAME=${await prompt("Initiator Name (leave blank if not using B2C/Reversal)")}
MPESA_INITIATOR_PASSWORD=${await prompt("Initiator Password (leave blank if not using B2C/Reversal)")}
...
MPESA_RESULT_URL=${await prompt("Result URL (for async APIs)", "https://yourdomain.com/api/mpesa/result")}
...
async function cmdDoctor() {
	...
	if (!!!(env["MPESA_INITIATOR_NAME"] && env["MPESA_INITIATOR_PASSWORD"])) console.log(dim("  — Not configured (only required for B2C, Reversal, Balance, Tax)"));
	else {
	...
	if (env["MPESA_RESULT_URL"]) console.log(`${g("✔")}  MPESA_RESULT_URL`);
	else console.log(dim("  — Not set"));
	console.log("");
...
async function cmdBalance(args) {
	...
	try {
		...
		if (result["ResponseCode"] === "0") console.log(`${g("✔  Balance query submitted!")} Result will be POSTed to:\n  ${c(resultUrl)}\n`);
		else {
